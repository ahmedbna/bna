...
export const ClubhouseReactions = ({ comment }: Props) => {
  const [open, setOpen] = useState(false);
  const commentReactions = useQuery(api.reactions.commentReactions, {
    clubhouseId: comment._id,
  });

  const react = useMutation(api.reactions.react);
  const handleEmojiSelect = async (emoji: string) => {
    await react({ clubhouseId: comment._id, reaction: emoji });
    setOpen(false);
  };
